...
class QwantVIPDailyStats: Codable {
    let date: Date
...
class QwantVIPGlobalStats {
    ...
    func appendStat(for domain: String, at date: Date = Date()) {
        if let idx = rawStats.firstIndex(where: { $0.date == date.noon }) {
            ...
            rawStats.append(QwantVIPDailyStats(domain: domain, date: date))
        }
