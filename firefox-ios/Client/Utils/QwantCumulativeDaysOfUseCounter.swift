...
/// Counter to know if a user has used the app a certain number of days
/// in a row, used for `QwantRatingPromptManager` requirements.
class QwantCumulativeDaysOfUseCounter {
    ...
    private(set) var hasRequiredCumulativeDaysOfUse: Bool {
        get { UserDefaults.standard.object(
            forKey: UserDefaultsKey.keyRequiredCumulativeDaysOfUseCount.rawValue) as? Bool ?? false
        }
        set { UserDefaults.standard.set(
            newValue,
            forKey: UserDefaultsKey.keyRequiredCumulativeDaysOfUseCount.rawValue)
        }
    }
