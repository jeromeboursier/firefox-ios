...
/// Counter to know if a user has the app installed for a certain number of days,
/// used for `QwantRatingPromptManager` requirements.
class QwantDaysAfterInstallCounter {
    ...
    private(set) var hasRequiredDaysAfterInstall: Bool {
        get { UserDefaults.standard.object(
            forKey: UserDefaultsKey.keyRequiredDaysAfterInstall.rawValue) as? Bool ?? false
        }
        set { UserDefaults.standard.set(
            newValue,
            forKey: UserDefaultsKey.keyRequiredDaysAfterInstall.rawValue)
        }
    }
