...
/// Counter to know if a user has used the app a certain number of days in a row,
...
class QwantUsesInDayCounter {
    ...
    private(set) var hasRequiredUsesInDay: Bool {
        get { UserDefaults.standard.object(
            forKey: UserDefaultsKey.keyRequiredUsesInDayCount.rawValue) as? Bool ?? false
        }
        set { UserDefaults.standard.set(
            newValue,
            forKey: UserDefaultsKey.keyRequiredUsesInDayCount.rawValue)
        }
    }
